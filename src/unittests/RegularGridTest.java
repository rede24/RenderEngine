...
public class RegularGridTest {
	...
	public void test() {
		...
		ImageWriter imageWriter = new ImageWriter("test grid malan sphere", 500, 500, 1000, 1000);
		Render render = new Render(imageWriter, scene).setDebugPrint().setMultithreading(3);// .setBox(4);
		render.renderImage();
		render.writeToImage();
	}
	...
	public void testForDanTheMaster() {
		...
		for (int i = 0; i <= 15; ++i)
			for (int j = 1; j <= 44; ++j) {
				geometries.add(new Polygon(Color.BLACK, new Material(0.5, 0.5, 100, 0.5, 0, 0.1 * (i % 2), 0),
...
//				new PointLight(new Color(103, 110, 13), new Point3D(0, -100, 0), 1, 0, 0));
		...
		ImageWriter imageWriter = new ImageWriter("FinalPictureWithBox", 1500, 1500, 1900, 1900);
		Render render = new Render(imageWriter, scene).setDebugPrint().setMultithreading(3).setBox(4);
		render.renderImage();
		render.writeToImage();
	}
